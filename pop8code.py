...
def pivoting_data(data, entry1, entry2, groupBy, sortBy):
    return (
        data[[entry1, entry2]]
...
def print_pivoted_data(data):
    print("------------------Sex--------------------")
    print(
        pivoting_data(
            train_df, "Sex", "Survived", "Sex", "Survived"
        ),
        "\n",
    )
    print("------------------Pclass--------------------")
    ...
    print(
        pivoting_data(
            train_df, "Age", "Survived", "Age", "Survived"
        ),
        "\n",
    )
    print("------------------Fare--------------------")
    print(
        pivoting_data(
            train_df, "Fare", "Survived", "Fare", "Survived"
        ),
        "\n",
    )


def visualize_numerical_correlation():
    grid = sns.FacetGrid(
        train_df,
        col="Survived",
        row="Pclass",
...
def visualize_scatter():
    grid = sns.FacetGrid(
        train_df, row="Embarked", size=3, aspect=2
    )
    grid.map(
        sns.pointplot,
        "Pclass",
...
def visualize_survived_correlation():
    survived = "survived"
...
def normalize_ages():
    for dataset in combine:
        ...
        rand_age = np.random.randint(
            mean - std, mean + std, size=is_null
        )
...
def set_age_boundaries():
    for dataset in combine:
        dataset.loc[dataset["Age"] <= 5, "Age"] = 0
        dataset.loc[
            (dataset["Age"] > 5) & (dataset["Age"] <= 16),
            "Age",
        ] = 1
        dataset.loc[
            (dataset["Age"] > 16) & (dataset["Age"] <= 32),
            "Age",
        ] = 2
        dataset.loc[
            (dataset["Age"] > 32) & (dataset["Age"] <= 48),
            "Age",
        ] = 3
        dataset.loc[
            (dataset["Age"] > 48) & (dataset["Age"] <= 64),
            "Age",
        ] = 4
...
def normalize_fare():
    ...
    for dataset in combine:
        ...
        dataset.loc[
            (dataset["Fare"] >= 9) & (dataset["Fare"] < 12),
            "Fare",
        ] = 1
        dataset.loc[
            (dataset["Fare"] >= 12)
...
def normalize_age_class():
    for dataset in combine:
        dataset["Age*Class*Fare"] = (
            dataset.Age * dataset.Pclass * dataset.Fare
        )
...
def classify_with_logistic_regression(
    trainingData, results, testData
):
    logreg = LogisticRegression()
    logreg.fit(trainingData, results)
    acc_log = round(
        logreg.score(trainingData, results) * 100, 2
    )
    return logreg.predict(testData), acc_log


def classify_with_SVM(trainingData, results, testData):
    svm = SVC()
    svm.fit(trainingData, results)
    acc_svm = round(
        svm.score(trainingData, results) * 100, 2
    )
    return svm.predict(testData), acc_svm


def classify_with_stochastic_gradient_descent(
    trainingData, results, testData
):
    sgd = SGDClassifier(max_iter=40, tol=None)
    sgd.fit(trainingData, results)
    acc_sgd = round(
        sgd.score(trainingData, results) * 100, 2
    )
...
def classify_with_random_forest(
    ...
    acc_random_forest = round(
        random_forest.score(trainingData, results) * 100, 2
    )
    return (
        random_forest.predict(testData),
        acc_random_forest,
    )
...
def main():
    ...
    visualize_scatter()
    visualize_survived_correlation()

    print_general_statistics(train_df)

    # Drop Useless Features
    train_df = train_df.drop(
        ["Ticket", "Cabin", "Name"], axis=1
    )
    ...
    test_df["Fare"].fillna(
        test_df["Fare"].dropna().median(), inplace=True
    )
    ...
    print_pivoted_data(train_df)

    combine = [train_df, test_df]
    print_general_information(train_df)
    ...
    X_test = test_df.drop(
        ["PassengerId", "Fare", "Age", "Pclass"], axis=1
    ).copy()
    ...
    [prediction2, acc_svm] = classify_with_SVM(
        X_train, Y_train, X_test
    )
    [
        prediction3,
        acc_sgd,
    ] = classify_with_stochastic_gradient_descent(
        X_train, Y_train, X_test
    )
    [
        prediction4,
